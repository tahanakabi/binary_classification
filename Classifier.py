...
class Classifier():
    def __init__(self, name, dataframe_number=1):
        ...
        df['Time sequence_C/W']=df['Time sequence_C']/df['W']
        ...
        df0=df.drop(['Folder','#Par'],1)
        df1=df[['Filename','Fluent?','Con', 'Pron','W','S']]
        df2=df[['Filename','Fluent?','Addition_C','Comparison_C','Concession_C','Contrast_C',
                               'Emphasis_C','Example_C','Summary_C','Time sequence_C','Subject_P',
                               'Object_P','Possessive_P','Relative_P','Demonstrative_P','Cause_C']]
        df3=df[['Filename','Fluent?','Average S.','Addition_C/W','Comparison_C/W','Concession_C/W','Contrast_C/W',
                                           'Emphasis_C/W','Example_C/W','Summary_C/W','Time sequence_C/W','Subject_P/W',
        ...
        self.features = self.df.drop(['Fluent?','Filename'],1).columns


    def separating_the_data(self,X,y,test_size):
...
#the number 4 hereunder refers to the third data frame defined before you can put 4,3,2,1 but don't use 1 for NB cause it contains dependent variables
nb_train=Classifier('data.csv', 4)
nb_train.separating_the_data(nb_train.X,nb_train.y,0.2)
